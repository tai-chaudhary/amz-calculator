...
import { INITIAL_INVENTORY } from './lib/inventoryData'
...
import {
  ...
  loadInventory, saveInventoryItem, saveAllInventory, deleteInventoryItem,
} from './lib/db'
...
const NAV = [
  ...
  { id: 'inventory', icon: '📦', label: "Homey's Inventory" },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function init() {
      setLoading(true)
      try {
        const [s, p, m, o, b, inv] = await Promise.all([loadSettings(), loadProducts(), loadMonths(), loadOverheads(), loadBin(), loadInventory()])
        setSettings(s)
        setSavedProducts(p)
        setMonths(m)
        setOverheads(o)
        ...
        if (inv === null || inv.length === 0) {
          // Seed with initial inventory from Excel
          await saveAllInventory(INITIAL_INVENTORY)
          setInventory(INITIAL_INVENTORY)
        } else {
          setInventory(inv.map(r => r.data || r))
        }
      } catch {
  ...
  const handleUpdateInventoryItem = useCallback(async (item) => {
    try {
  ...
  const handleDeleteInventoryItem = useCallback(async (id) => {
    try {
  ...
  const handleSendToCalculator = useCallback((item) => {
    setCalc({ ...EMPTY_CALC, ...item, costPrice: item.costPrice })
    setLoadedProductId(null)
    setLoadedProductName('')
    setPage('calculator')
    showToast(`"${item.name}" loaded into calculator`)
  }, [])
  ...
  const mainNavItems = NAV.slice(0, 6)
  const configNavItems = NAV.slice(6)
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          {settings && (
            <>
              ...
              <div style={{display: page === 'inventory' ? 'block' : 'none'}}>
                <InventoryPage
                  inventory={inventory}
                  settings={settings}
