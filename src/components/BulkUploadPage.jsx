...
import { EmptyState, Icon, PageHeader, ProductImage } from './UI'
...
export default function BulkUploadPage({ settings, stockItems = [], onBulkImport }) {
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Catalogue / import"
        title="Bulk Upload"
        description="Bring a prepared catalogue into the portal, validate each commercial model, then choose whether each listing stays saved, enters review or goes live."
        meta="CSV import · validation before commit"
        actions={
          <>
            <button className="btn btn-secondary btn-sm" onClick={() => setShowTemplate(t => !t)}>{showTemplate ? 'Hide' : 'View'} field guide</button>
            <button className="btn btn-secondary btn-sm" onClick={() => downloadText('how-to-fill-the-template.txt', generateAiBrief(settings))}><Icon name="list" size={14} /> Filling instructions</button>
            <button className="btn btn-primary btn-sm" onClick={() => downloadCsv('product-upload-template.csv', generateTemplateCsv())}><Icon name="download" size={14} /> Download template</button>
          </>
        }
      />
      ...
      {showTemplate && (
        <div className="card mb-5">
          ...
          <div className="text-xs text-ink/45 mt-3 bg-paper rounded-[5px] p-3">
      ...
      {!parsed && (
        <div
          className="card border-2 border-dashed border-rule hover:border-royal-300 transition-colors cursor-pointer" role="button" tabIndex={0} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); (() => fileRef.current?.click())() } }} onClick={() => fileRef.current?.click()}
          onDrop={handleDrop}
        ...
        >
          <EmptyState
            ...
            sub="Use the Good & General upload template so carrier, packaging and review fields map cleanly."
          />
          <input
            ref={fileRef}
            type="file"
