...
/**
 * Lets a listing be built from stock items. Handles searching existing items
...
export default function ComponentPicker({ components = [], stockItems, onChange, onCreateStockItem, asin }) {
  ...
  return (
    <div className="p-3 bg-royal-50 border border-royal-100 rounded-xl">
      ...
      {components.length > 0 && (
        <div className="space-y-1.5 mb-3">
          {components.map((c, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2 bg-white rounded-lg px-2.5 py-2 border border-rule">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-ink truncate">
                    {si ? si.name : <span className="text-loss">Missing stock item</span>}
                  </div>
      ...
      {!creating && (
        <div className="relative">
          <input
            ...
            placeholder={components.length ? 'Add another product…' : 'Search your stock items…'}
            value={search}
            onChange={e => setSearch(e.target.value)}
          />

          {search.trim() && (
            <div className="absolute z-20 left-0 right-0 mt-1 bg-white border border-rule rounded-xl shadow-lg overflow-hidden">
              {matches.map(si => {
                const d = si.data || {}
                return (
                  <button
              ...
              >
                + Create "{search.trim()}" as a new stock item
              </button>
            </div>
          )}
        </div>
      ...
      {/* Inline creation */}
      {creating && (
        <div className="bg-white border border-royal-200 rounded-xl p-3">
          <div className="text-xs font-semibold text-ink/70 mb-2">New stock item</div>
      ...
      {components.length === 0 && !creating && (
        <div className="text-xs text-ink/55 mt-2">
          ...
          A single item is one stock item × 1; a 3-pack is one × 3; a mixed bundle is several.
        </div>
      )}
    </div>
  )
