...
/**
 * Groups the catalogue by brand or supplier, showing stock items and listings
 * as two distinct things rather than conflating them.
 *
 * Supplier and brand live on the stock item, so grouping walks from stock items
    ...
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-ink/50 mt-1">
            {groups.length} {groups.length === 1 ? label : title.toLowerCase()} ·
            {' '}{stockItems.length} stock item{stockItems.length !== 1 ? 's' : ''} ·
        ...
        <button
          ...
          onClick={() => exportRowsToCsv(stampedName(title.toLowerCase()), listings, [
            ...
            { header: 'Stock items', value: x => stockFor(x.p).map(s => `${s.qty}x ${s.si.name}`).join(' + ') },
      ...
      {groups.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <div className="font-medium text-ink/80 mb-1">Nothing to show yet</div>
          <div className="text-sm text-ink/45">{emptyHint}</div>
        </div>
      ) : (
        <>
          <div className="flex gap-3 flex-wrap items-end mb-4">
            <div>
              ...
              <input type="text" placeholder={`Search ${title.toLowerCase()}…`} value={search}
                onChange={e => setSearch(e.target.value)} className="input text-sm" style={{ width: 240 }} />
            </div>
            <div>
              <label className="label text-xs">Sort by</label>
              <select className="input text-sm" style={{ width: 195 }} value={sort} onChange={e => setSort(e.target.value)}>
                <option value="listings">Most listings</option>
                <option value="stock">Most stock items</option>
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs mt-0.5 flex gap-2 flex-wrap items-center">
                            <span className="text-royal-500 font-medium">{g.stock.length} stock item{g.stock.length !== 1 ? 's' : ''}</span>
                  ...
                  {isOpen && (
                    <div className="border-t border-rule">
                      <div className="flex gap-1 px-4 pt-3">
                        <button
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === 'listings' ? 'bg-royal-100 text-royal-700' : 'text-ink/45 hover:text-ink/70'}`}
                          onClick={() => setTab('listings')}
                        >Listings ({g.listings.length})</button>
                        <button
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tab === 'stock' ? 'bg-royal-100 text-royal-700' : 'text-ink/45 hover:text-ink/70'}`}
                          onClick={() => setTab('stock')}
                        >Stock items ({g.stock.length})</button>
                      </div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {[...rows].sort((a, b) => b.r.margin - a.r.margin).map(({ lp, saved, p, r }) => (
            <tr key={lp.id} className="border-b border-rule/60 hover:bg-royal-50/40 cursor-pointer"
              ...
              <td className="px-5 py-2.5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <div className="font-medium text-ink truncate hover:text-royal-600" style={{ maxWidth: 260 }}>{saved.name}</div>
                    <div className="text-xs text-ink/45 flex gap-2">
                      {p.asin && (
                        <a href={`https://www.amazon.co.uk/dp/${p.asin}`} target="_blank" rel="noopener noreferrer"
                          className="text-royal-400 hover:text-royal-600" onClick={e => e.stopPropagation()}>{p.asin} ↗</a>
                      )}
...
function StockTable({ rows, listings, stockFor, onNavigate }) {
  if (!rows.length) return <div className="px-5 py-6 text-sm text-ink/45 text-center">No stock items here</div>
  ...
  return (
    <div className="overflow-x-auto mt-2">
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-paper border-y border-rule">
            <th className="text-left px-5 py-2.5 text-xs font-semibold text-ink/45">Stock item</th>
        ...
        <tbody>
          {[...rows].sort((a, b) => a.name.localeCompare(b.name)).map(si => {
            ...
            return (
              <tr key={si.id} className="border-b border-rule/60 hover:bg-royal-50/40 cursor-pointer"
                ...
                <td className="px-5 py-2.5">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="font-medium text-ink truncate hover:text-royal-600" style={{ maxWidth: 280 }}>{si.name}</div>
                  </div>
                </td>
