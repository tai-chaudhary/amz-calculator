...
import { EmptyState, Spinner, ResultRow, Modal, ProductImage } from './UI'
...
export default function SavedPage({ savedProducts, stockItems = [], settings, focusId, onNavigate, liveProducts = [], onDelete, onLoad, onRename, onPushLive, onSendReview }) {
  ...
  React.useEffect(() => {
    ...
    setSearch('')
    const t = setTimeout(() => {
      document.getElementById(`saved-${focusId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 80)
  ...
  if (!savedProducts.length) return (
    <div>
      <h1 className="text-xl font-semibold text-ink mb-6">Saved Products</h1>
  ...
  const filtered = search.trim()
    ? savedProducts.filter(row => {
        const q = search.toLowerCase()
        return row.name.toLowerCase().includes(q) ||
          (row.data?.brand || '').toLowerCase().includes(q) ||
          (row.data?.asin || '').toLowerCase().includes(q) ||
          (row.data?.supplierSku || '').toLowerCase().includes(q) ||
          (row.data?.supplierName || '').toLowerCase().includes(q)
      })
    : savedProducts
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          <h1 className="text-[28px] leading-tight">Saved Products</h1>
          <p className="text-sm text-ink/50 mt-1">
            {search.trim() ? `${sorted.length} of ${savedProducts.length} products` : `${savedProducts.length} product${savedProducts.length !== 1 ? 's' : ''} saved`}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            onClick={() => exportRowsToCsv(stampedName('saved-products'), sorted, [
              ...
              { header: 'Bundle Qty', value: row => row.data?.bundleQty || 1 },
              ...
              { header: 'Review Status', value: row => row.data?.reviewStatus || 'none' },
              { header: 'Saved', value: row => new Date(row.created_at).toLocaleDateString('en-GB') },
            ])}
          >⬇ Export CSV</button>
          <label className="text-xs text-ink/55">Sort by</label>
      ...
      <div className="mb-4">
        <input
          type="text"
          placeholder="Search by name, brand, ASIN or supplier SKU…"
          value={search}
        ...
        {search.trim() && sorted.length === 0 && (
          <p className="text-sm text-ink/45 mt-2">No products match "{search}"</p>
        )}
      </div>
      ...
      <div className="space-y-4">
        {sorted.map((row) => {
          ...
          return (
            <div key={row.id} id={`saved-${row.id}`}
              className={`card transition-colors ${focusId === row.id ? 'ring-2 ring-royal-300 border-royal-200' : ''}`}>
              <div className="flex items-start justify-between mb-4 flex-wrap gap-2">
                <div className="w-14 h-14 rounded-xl border border-rule bg-white flex-shrink-0 mr-1 flex items-center justify-center overflow-hidden">
                ...
                <div className="flex-1">
                  {renamingId === row.id ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        className="text-ink/30 hover:text-ink/55 text-xs"
                        onClick={() => { setRenamingId(row.id); setRenameVal(row.name) }}
                        title="Rename"
                      >✏️</button>
                    </div>
                  )}
                  <div className="text-xs text-ink/45 mt-0.5 flex flex-wrap gap-x-2 items-center">
                    {p.brand && <span className="pill pill-brand">{p.brand}</span>}
                    {p.reviewStatus === 'review' && <span className="pill pill-paused">🔍 In review</span>}
                    {parseInt(p.bundleQty) > 1 && !p.weightPerItem && !p.weightOverride && (
                      <span className="pill pill-alert" title="Bundle weight needs checking — shipping may be understated">
                        ⚠ Check weight
                      </span>
                    )}
                    <span>Sell: {fmt(parseFloat(p.sellPrice) || 0)}</span>
                    {parseInt(p.bundleQty) > 1
                      ? <span className="text-royal-500 font-medium">{p.bundleQty} × {fmt(parseFloat(p.costPerItem)||0)} = {fmt((parseFloat(p.costPerItem)||0)*(parseInt(p.bundleQty)||1))}</span>
                      : <span>Cost: {fmt(parseFloat(p.costPrice) || 0)}</span>
                    }
                    <span>· Ref: {p.refFee}%</span>
                    {parseInt(p.bundleQty) > 1 && <span className="pill pill-brand">Bundle ×{p.bundleQty}</span>}
                    {p.createdByName && <span className="text-ink/45">· added by {p.createdByName}</span>}
                    <span className="cursor-pointer underline text-royal-400" onClick={() => setDetailProduct(row)}>View details</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div className="metric-card">
                  ...
                  <div className={`metric-value ${r.netProfit >= 0 ? 'text-gain' : 'text-loss'}`}>{fmtSigned(r.netProfit)}</div>
                </div>
                <div className="metric-card">
              ...
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <div className="text-xs text-ink/55">Monthly forecast:</div>
                <input
                  type="number"
                ...
                {u > 0 && (
                  <div className="flex gap-3 text-sm flex-wrap">
                    <span className={`font-medium ${r.netProfit * u >= 0 ? 'text-gain' : 'text-loss'}`}>{fmtSigned(r.netProfit * u)} profit / mo</span>
                    <span className="text-ink/45">·</span>
                    <span className="text-blue-600">{fmt(r.vatAmount * u)} VAT / mo</span>
                    <span className="text-ink/45">·</span>
                    <span className="text-ink/70">{fmt(r.sellPrice * u)} revenue / mo</span>
                  </div>
                )}
              </div>

              <div className="border-t border-rule pt-3 flex gap-2">
                <button className="btn btn-secondary btn-sm" onClick={() => onLoad(row)}>✏️ Edit in calculator</button>
                {onPushLive && (() => {
                  const isLive = liveProducts.some(lp => lp.saved_product_id === row.id && lp.status !== 'removed')
                  return (
                    <button
                      className={`btn btn-sm ${isLive ? 'btn-secondary opacity-60' : 'btn-success'}`}
                      onClick={() => !isLive && onPushLive(row.id, row.name)}
                      disabled={isLive}
                    >
                      {isLive ? '🟢 Live' : '🚀 Go Live'}
                    </button>
                ...
                {onSendReview && (p.reviewStatus === 'review'
                  ? <button className="btn btn-sm btn-secondary opacity-60" disabled>🔍 In review</button>
                  : <button className="btn btn-sm btn-secondary" onClick={() => onSendReview(row.id, row.name)}>🔍 Send to review</button>
                )}
...
function ProductDetailModal({ row, settings, stockItems = [], onClose, onLoad }) {
  const p = row.data
  const r = calcProduct(p, settings.carriers, settings.packaging, stockItems)
    ...
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-lg my-8 shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-rule">
          <div>
            <div className="font-semibold text-lg text-ink">{row.name}</div>
            <div className="text-xs text-ink/45 mt-0.5">Full product details</div>
          </div>
          <button onClick={onClose} className="text-ink/45 hover:text-ink/70 text-2xl leading-none">×</button>
        </div>
        <div className="px-6 py-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            <div className="metric-card"><div className="metric-label">Net Profit</div><div className={`metric-value ${r.netProfit >= 0 ? 'text-gain' : 'text-loss'}`}>{fmtSigned(r.netProfit)}</div></div>
          ...
          {(p.brand || p.asin || p.supplierName || p.supplierSku || p.productUrl || p.notes) && (
            <>
              <div className="text-xs font-semibold text-ink/45 mb-2">Product Info</div>
              ...
              {p.productUrl && (
                <div className="result-row">
                  <span className="text-ink/55">Amazon URL</span>
                  <a href={p.productUrl} target="_blank" rel="noopener noreferrer" className="text-royal-500 text-sm hover:underline">View listing ↗</a>
                </div>
              )}
          ...
          <div className="text-xs font-semibold text-ink/45 mt-4 mb-2">Pricing</div>
          ...
          <div className="text-xs font-semibold text-ink/45 mt-4 mb-2">Shipping</div>
          ...
          <div className="text-xs font-semibold text-ink/45 mt-4 mb-2">P&L breakdown</div>
          ...
          <ResultRow label="Referral fee" value={`-${fmt(r.referralFee)}`} valueClass="text-loss" />
          ...
          <div className="flex justify-between items-center pt-3 mt-1 border-t border-rule">
            <span className="font-semibold text-ink">Net profit</span>
            <span className={`font-bold text-base ${r.netProfit >= 0 ? 'text-gain' : 'text-loss'}`}>{fmtSigned(r.netProfit)}</span>
          </div>
        </div>
