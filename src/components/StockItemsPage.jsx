...
import { SupplierLink, BrandLink, AmazonLink } from './Links'

export default function StockItemsPage({
  stockItems, savedProducts, liveProducts, settings, focusId, onNavigate,
  onUpdateStockItem, onDeleteStockItem, onRestoreStockItem, onRunMigration, onAddStockItem, onCreateListing,
}) {
  ...
  // Which listings use each product
  const usage = useMemo(() => {
    const map = new Map()
  ...
  const rows = stockItems.map(si => {
    ...
    // Products have no ASIN of their own — borrow one from a linked listing
  ...
  const archivedCount = rows.filter(r => r.d.archived).length
  const filtered = rows.filter(({ si, d, uses, liveUses }) => {
    // Archived items only appear when asked for
    if (usageFilter === 'archived') return !!d.archived
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Products"
        description="Your purchasing layer: physical items, supplier references and costs shared across one or more Amazon listings."
        meta={`${stockItems.length} product${stockItems.length !== 1 ? 's' : ''} · ${savedProducts.length} listing${savedProducts.length !== 1 ? 's' : ''} modelled`}
        actions={
          <>
            <button className="btn btn-primary btn-sm" onClick={() => setAdding(true)}><Icon name="plus" size={14} /> Add product</button>
            <button
              className="btn btn-secondary btn-sm"
              disabled={sorted.length === 0}
              onClick={() => exportRowsToCsv(stampedName('stock-items'), sorted, [
                { header: 'Product', value: x => x.si.name },
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <div className="font-semibold text-ink">
                {unlinkedCount} listing{unlinkedCount !== 1 ? 's are' : ' is'} not linked to a product
              </div>
              <div className="text-sm text-ink/55 mt-0.5">
                Link them so a cost change on the product updates every listing that uses it.
              </div>
            </div>
      ...
      {stockItems.length === 0 && unlinkedCount === 0 ? (
        <div className="surface">
          <EmptyState icon="package" title="No products yet" sub="Products are the physical products you buy from suppliers and reuse across listing models." action={<button className="btn btn-primary btn-sm" onClick={() => setAdding(true)}>Add product</button>} />
        </div>
      ) : stockItems.length > 0 && (
        <>
          <div className="filter-bar mb-4">
            <SearchInput value={search} onChange={e => setSearch(e.target.value)} placeholder="Name, SKU, supplier or linked listing…" className="w-full sm:max-w-[320px]" />
            <select className="input text-sm sm:w-[175px]" value={sort} onChange={e => setSort(e.target.value)} aria-label="Sort products">
            ...
            <select className="input text-sm sm:w-[165px]" value={supplierFilter} onChange={e => setSupplierFilter(e.target.value)} aria-label="Supplier filter">
              <option value="">All suppliers</option>
              {suppliers.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <select className="input text-sm sm:w-[145px]" value={brandFilter} onChange={e => setBrandFilter(e.target.value)} aria-label="Brand filter">
              <option value="">All brands</option>
              {brands.map(b => <option key={b} value={b}>{b}</option>)}
            </select>
            <select className="input text-sm sm:w-[170px]" value={usageFilter} onChange={e => setUsageFilter(e.target.value)} aria-label="Usage filter">
              ...
              {archivedCount > 0 && <option value="archived">Archived ({archivedCount})</option>}
            </select>
            {(search || supplierFilter || brandFilter || usageFilter) && <button className="btn btn-secondary btn-sm" onClick={() => { setSearch(''); setSupplierFilter(''); setBrandFilter(''); setUsageFilter('') }}>Clear</button>}
            <div className="text-xs text-ink/45 sm:ml-auto">{filtered.length} of {rows.length}</div>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-rule bg-paper">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-ink/45">Product</th>
                    ...
                    <tr id={`stock-${si.id}`}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          >{uses.length ? <Icon name={isOpen ? 'chevronDown' : 'chevronRight'} size={14} /> : null}</button>
                          <div className="w-11 h-11 rounded-[5px] border border-rule bg-white flex items-center justify-center flex-shrink-0 overflow-hidden">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-ink/45 flex items-center gap-1.5 flex-wrap mt-0.5">
                              {d.brand && <BrandLink name={d.brand} />}
                              {d.supplierSku ? <span>SKU {d.supplierSku}</span> : <span className="text-warn">no SKU</span>}
                              {d.barcode && <span>· {d.barcode}</span>}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-3 text-ink/70">{d.supplierName ? <SupplierLink name={d.supplierName} /> : <span className="text-ink/30">—</span>}</td>
                      ...
                      <td className="px-3 py-3 text-right">
                        <div className="flex gap-1 justify-end">
                          {!d.archived && onCreateListing && (
                            <button className="btn btn-xs btn-primary" title="Start a new listing for this product" onClick={() => onCreateListing(si, 1)}>
                              <Icon name="plus" size={12} /> Listing
                            </button>
                          )}
                          <button className="btn btn-xs btn-secondary" onClick={() => setEditing({ si, d, uses })}>Edit</button>
                          {d.archived ? (
                            <button className="btn btn-xs btn-secondary" onClick={() => onRestoreStockItem?.(si.id)}>Restore</button>
                          ) : uses.length === 0 ? (
                            <button className="btn btn-xs btn-danger" aria-label={`Delete ${si.name}`}
                              onClick={() => { if (window.confirm(`Delete "${si.name}"? It isn't used by any listing.`)) onDeleteStockItem(si.id) }}>
                              <Icon name="trash" size={12} />
                            </button>
                          ) : (
                            <button className="btn btn-xs btn-secondary" aria-label={`Archive ${si.name}`}
                              onClick={() => { if (window.confirm(`Archive "${si.name}"? ${uses.length} listing${uses.length !== 1 ? 's use' : ' uses'} it, so it will be kept for their calculations but hidden from new listings.`)) onDeleteStockItem(si.id) }}>
                    ...
                    {isOpen && (
                      <tr className="bg-paper border-b border-rule">
                        <td colSpan={6} className="px-4 pb-4 pt-0">
                          <div className="bg-white rounded-[6px] border border-rule overflow-hidden ml-7">
                            <div className="px-3 py-2 bg-paper border-b border-rule text-xs font-semibold text-ink/45">
                              Listings using this product
                            </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                          ...
                                          <div className="min-w-0">
                                            ...
                                            <div className="text-ink/45">
                                              {u.p.asin ? <AmazonLink asin={u.p.asin} /> : 'no ASIN'}
                                            </div>
                                          </div>
                                        </div>
                                      </td>
...
function StockItemModal({ item, data, uses, stockItems, settings, liveProducts, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-[7px] w-full max-w-3xl my-8 shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-rule">
          <div>
            <div className="font-semibold text-base text-ink">{item ? item.name : 'New product'}</div>
            {uses.length > 0 && (
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {/* Impact preview */}
          {impact.length > 0 && (
            <div className="border-2 border-warn/25 bg-warn/5 rounded-[6px] p-4">
              ...
              <div className="overflow-x-auto bg-white rounded-[5px]">
                <table className="w-full text-xs">
                  <thead>
                    ...
                    {impact.map(i => {
                      ...
                      return (
                        <tr key={i.row.id} className={`border-b border-rule/60 ${broke ? 'bg-loss/5' : ''}`}>
                          <td className="px-3 py-2">
                            <div className="text-ink truncate" style={{ maxWidth: "min(100%, 260px)" }}>{i.row.name}</div>
          ...
          {impact.length === 0 && uses.length > 0 && (
            <div className="bg-paper rounded-[6px] p-3">
              ...
              <div className="space-y-1">
                {uses.map(u => (
                  <div key={u.row.id} className="flex justify-between text-xs">
                    <span className="text-ink/80 truncate" style={{ maxWidth: "min(100%, 380px)" }}>{u.row.name}</span>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-[7px] w-full max-w-3xl my-8 shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-rule">
          <div>
            <div className="font-semibold text-base text-ink">Link listings to products</div>
        ...
        <div className="px-6 py-4 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="metric-card"><div className="metric-label">Products</div><div className="metric-value text-lg">{plan.stockItems.length}</div></div>
          ...
          <div className="text-sm text-ink/55 bg-paper rounded-[6px] p-3">
            Listings are grouped by Supplier SKU, or by product name and supplier where no SKU is set.
            A listing's per-unit cost becomes the product's cost. Existing listings keep their own
            figures until you confirm.
          </div>

          {plan.conflicts.length > 0 && (
            <div className="border border-warn/25 bg-warn/5 rounded-[6px] p-3">
          ...
          {plan.mixed.length > 0 && (
            <div className="border border-royal-200 bg-royal-50 rounded-[6px] p-3">
          ...
          {plan.unlinkable.length > 0 && (
            <div className="border border-rule rounded-[6px] p-3">
          ...
          <details className="text-sm">
            <summary className="cursor-pointer text-royal-500 hover:text-royal-700 font-medium">
              Preview all {plan.stockItems.length} products
            </summary>
            <div className="mt-2 max-h-72 overflow-y-auto border border-rule rounded-[5px]">
              <table className="w-full text-xs">
                <thead className="bg-paper sticky top-0">
                  <tr>
                    <th className="text-left px-3 py-2 font-semibold text-ink/45 uppercase">Product</th>
        ...
        <div className="px-6 pb-6 flex gap-2">
          <button className="btn btn-primary flex-1 justify-center" onClick={() => onConfirm(plan)}>
            Create {plan.stockItems.length} products &amp; link {plan.links.length} listings
          </button>
          <button className="btn btn-secondary" onClick={onClose}>Cancel</button>
        </div>
      </div>
