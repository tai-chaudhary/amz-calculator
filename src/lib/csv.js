...
export const CSV_COLUMNS = [
  ...
  { key: 'serviceLevel', label: 'Service Level',     required: false, example: 'standard',                         note: 'standard / nextday / prime' },
...
export function exportRowsToCsv(filename, rows, columns) {
  const esc = (v) => {
    ...
    // A cell starting with = + - @ would run as a formula when the file is
    // opened in Excel. Prefix it so it's read as text. Plain negative numbers are fine.
