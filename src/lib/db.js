...
export async function loadInventory() {
  ...
  if (error) return null // null = not seeded yet
  return data || []
}
...
export async function saveInventoryItem(item) {
  const { data, error } = await supabase
    .from('inventory')
    .upsert({ id: item.id, name: item.name, data: item }, { onConflict: 'id' })
    .select()
    .single()
  if (error) throw error
  return data
...
export async function saveAllInventory(items) {
  const rows = items.map(item => ({ id: item.id, name: item.name, data: item }))
